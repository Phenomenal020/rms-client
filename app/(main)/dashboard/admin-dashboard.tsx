...
export function AdminDashboard() {
    ...
    // Shared refresh — called after any successful edit or access change
    function refreshUsers() {
        authClient.admin.listUsers({ query: {} }).then(({ data }) => {
            if (data) setUsers(data.users as AppUser[]);
        });
    }

    // Effect to fetch the users
    useEffect(() => {
        authClient.admin.listUsers({ query: {} }).then(({ data }) => {
            if (data) setUsers(data.users as AppUser[]);
            setLoading(false);
