...
export default async function SubjectsSettingsPage() {
  ...
  // if there is no session user, then return the unauthorised page (unauthenticated user)
  ...
  type AssessmentStructure = Prisma.AssessmentStructureGetPayload<{}>;
  
  let subjects: Subject[] = [];
  let assessmentStructure: AssessmentStructure[] = [];
  ...
  if (user?.school) {
    ...
    if (term) {
      ...
      // If no academic term exists, initialse with empty arrays
      subjects = [];
      assessmentStructure = [];
    }
  }
